...
export const YourNature: React.FC<{ stem: StemImageProfile }> = ({ stem }) => (
  <section className="grid items-center gap-10 md:grid-cols-[auto_1fr]">
    ...
    <div className="flex flex-col items-center">
      <div className={`flex h-36 w-36 items-center justify-center rounded-full bg-white/50 ring-1 ring-ink/5 ${ELEMENT_COLORS[stem.element]}`}>
    ...
    <div>
      <p className="font-sans text-xs font-semibold uppercase tracking-[0.3em] text-sage-deep">Your nature</p>
      ...
      <div className="mt-8 grid gap-6 sm:grid-cols-2">
        <div>
          <h3 className="mb-3 font-sans text-[11px] font-semibold uppercase tracking-widest text-sage-deep">What you bring</h3>
          <ul className="space-y-2">
            {stem.strengths.map((s) => (
              <li key={s} className="flex gap-2 text-sm text-ink/80">
                <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-sage" />{s}
              </li>
            ))}
          </ul>
        </div>
